...
namespace MKB.FinancialMethods
{
  public enum DueDate
  {
  ...
  /// <summary>The <see langword="Financial" /> class contains procedures used to perform financial operations. </summary>
  public sealed class Financial
  {
    ...
    public static double DDB(double Cost, double Salvage, double Life, double Period, double Factor = 2.0)
    {
      if (Factor <= 0.0 || Salvage < 0.0 || (Period <= 0.0 || Period > Life))
        throw new ArgumentException("Argument '{0}' is not a valid value.", nameof(Factor));
      if (Cost <= 0.0)
        return 0.0;
      if (Life < 2.0)
        return Cost - Salvage;
    ...
    public static double IPmt(double Rate, double Per, double NPer, double PV, double FV = 0.0, DueDate Due = DueDate.EndOfPeriod)
    {
      ...
      if (Per <= 0.0 || Per >= NPer + 1.0)
        throw new ArgumentException("Argument '{0}' is not a valid value.", nameof(Per));
    ...
    public static double MIRR(ref double[] ValueArray, double FinanceRate, double ReinvestRate)
    {
      if (ValueArray.Rank != 1)
        throw new ArgumentException("Argument '{0}' cannot be a multi-dimensional array.", nameof(ValueArray));
      int num1 = 0;
      int num2 = checked(ValueArray.GetUpperBound(0) - num1 + 1);
      if (FinanceRate == -1.0)
        throw new ArgumentException("Argument '{0}' is not a valid value.", nameof(FinanceRate));
      if (ReinvestRate == -1.0)
        throw new ArgumentException("Argument '{0}' is not a valid value.", nameof(ReinvestRate));
      if (num2 <= 1)
        throw new ArgumentException("Argument '{0}' is not a valid value.", nameof(ValueArray));
      double num3 = LDoNPV(FinanceRate, ref ValueArray, -1);
      if (num3 == 0.0)
        throw new DivideByZeroException("Division by zero.");
      double x = -LDoNPV(ReinvestRate, ref ValueArray, 1) * Math.Pow(ReinvestRate + 1.0, (double)num2) / (num3 * (FinanceRate + 1.0));
      if (x < 0.0)
    ...
    {
      if (Rate <= -1.0)
        throw new ArgumentException("Argument '{0}' is not a valid value.", nameof(Rate));
      if (Rate == 0.0)
      {
        if (Pmt == 0.0)
          throw new ArgumentException("Argument '{0}' is not a valid value.", nameof(Pmt));
        return -(PV + FV) / Pmt;
      }
    ...
    {
      if (ValueArray == null)
        throw new ArgumentException("Argument '{0}' is Nothing.", nameof(ValueArray));
      if (ValueArray.Rank != 1)
        throw new ArgumentException("Argument '{0}' cannot be a multi-dimensional array.", nameof(ValueArray));
      int num1 = 0;
      int num2 = checked(ValueArray.GetUpperBound(0) - num1 + 1);
    ...
    {
      if (NPer == 0.0)
        throw new ArgumentException("Argument '{0}' is not a valid value.", nameof(NPer));
      if (Rate == 0.0)
        return (-FV - PV) / NPer;
      double num1 = Due == DueDate.EndOfPeriod ? 1.0 : 1.0 + Rate;
    ...
    {
      if (NPer <= 0.0)
        throw new ArgumentException("Argument 'NPer' must be greater than zero.");
      double Rate1 = Guess;
      ...
      while (num3 <= 39);
      throw new ArgumentException("Cannot calculate rate using the arguments provided.");
    }
    ...
    {
      if (Salvage < 0.0)
        throw new ArgumentException("Argument '{0}' must be greater than or equal to zero.", nameof(Salvage));
      if (Period > Life)
        throw new ArgumentException("Argument 'Period' must be less than or equal to argument 'Life'.");
      if (Period <= 0.0)
        throw new ArgumentException("Argument '{0}' must be greater than zero.", nameof(Period));
